...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        # Create a new node
        new_node = (id, name.replace(',', ''))

        # Check if the new node already exists in the nodes list
    ...
    def max_degree_nodes(self) -> dict:
        ...
        # Iterate over each edge to calculate the degree of each node
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        path = f'/3/movie/{movie_id}/credits?api_key={self.api_key}&language=en-US'
        ...
        if exclude_ids:
            cast = list(filter(lambda elem: elem['id'] not in exclude_ids, cast))
        # Limit the cast member
        if limit:
            cast = list(filter(lambda elem: elem['order'] < limit, cast))
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        # connect to api
        conn = http.client.HTTPSConnection('api.themoviedb.org')
        path = f'/3/person/{person_id}/movie_credits?api_key={self.api_key}&language=en-US'
        conn.request('GET', path)
        response = conn.getresponse()
        ...
        if vote_avg_threshold:
            cast = list(filter(lambda elem: elem['vote_average'] >= vote_avg_threshold, cast))
        
        return cast
    # Add co-actors of a given actor to the graph.
    def add_coactors(self, node:tuple, exclude:list=None)->list:
        # Get high-rated movie credits for the person
        movie_credits = self.get_movie_credits_for_person(node[0], vote_avg_threshold=8.0)
        new_nodes = []

         # Iterate over each movie
        for movie in movie_credits:
            cast_members = self.get_movie_cast(str(movie['id']), limit=3, exclude_ids=exclude)
            movie_nodes = []
            # Iterate over each cast member
            for member in cast_members:
                new_node = (str(member['id']), member['name'])
                # Add the new actor
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='5a616550c811b1838366b3a4b35c0ce2')
